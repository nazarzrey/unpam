...
if ($result->num_rows > 0) {
    while($row = $result->fetch_assoc()) {
        ...
        if ($row['exif_data'] !== null) {
            $exifDataJson = $row['exif_data'];
            echo "Data JSON dari kolom exif_data: " . $exifDataJson . "<br>";
            
            // Coba untuk mendekode JSON
            $exifData = json_decode($exifDataJson, true);
            if ($exifData !== null) {
                // Data JSON berhasil diuraikan
                ...
                // Gagal mendekode JSON
                $output .= '<p>Tidak dapat memproses Exif Data. Data JSON tidak valid.</p>';
            }
        } else {
